...
def test_is_shared_array():
    shared_array = pyn_array.shared.make_shared_array((6,), np.int64, zeros=True)

    assert pyn_array.shared.is_shared_array(shared_array)
    assert pyn_array.shared.is_shared_array(shared_array[2:]) # a view can still be referenced
    assert not pyn_array.shared.is_shared_array(np.asarray(shared_array))
    assert not pyn_array.shared.is_shared_array(np.arange(6))
    assert not pyn_array.shared.is_shared_array(pyn_array.SimArray(np.arange(6)))
