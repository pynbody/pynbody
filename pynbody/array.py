...
def _array_factory(dims, dtype, zeros, shared):
    ...
    if shared and posix_ipc:
        ...
        mem = posix_ipc.SharedMemory(
            fname, posix_ipc.O_CREX, size=int(np.dtype(dtype).itemsize * size))
...
if posix_ipc:
    ...
    def _shared_array_deconstruct(ar, transfer_ownership=False):
        ...
        assert hasattr(
            ar_base, '_shared_fname'), "Cannot prepare an array for shared use unless it was created in shared memory"

        ownership_out = transfer_ownership and ar_base._shared_del
        if transfer_ownership:
            ar_base._shared_del = False

        offset = ar.__array_interface__['data'][
            0] - ar_base.__array_interface__['data'][0]
