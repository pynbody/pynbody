...
def render_image(int nx, int ny,
    ...
    cdef image_output_type* c_result = <image_output_type*>result.data
            ...
            if (max_d_over_h*sm_i/pixel_dx<1 and max_d_over_h*sm_i/pixel_dy<1) :
                ...
                for x_pos in range(x_pix_start, x_pix_stop) :
                    ...
                    for y_pos in range(y_pix_start, y_pix_stop) :
                        ...
                        # could this be faster? It seems to be FAR faster but only when using stack-allocated memory for c_result?? -->
    ...
    print "finished processing",total_ptcls
    return result
