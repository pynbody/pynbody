...
def _interpolated_renderer(fn, levels):
    ...
    def render_fn(*args, **kwargs):
        ...
        for i in xrange(1, levels):
            ...
            base += scipy.ndimage.interpolation.zoom(
                new_im, float(base.shape[0]) / new_im.shape[0], order=1)
        return base
    return render_fn
...
def spectra(snap, qty='rho', x1=0.0, y1=0.0, v2=400, nvel=200, v1=None,
    ...
    *smooth*: The name of the array which contains the smoothing lengths
    ...
    conv_ratio = (qty.units * mass.units / (rho.units * sm.units ** kernel.h_power)
                  ).ratio(str(nnucleons) + ' m_p cm^-2', **x.conversion_context())
    ...
    x, y, vz, temp, sm, qty, mass, rho = [
        q.view(np.ndarray) for q in x, y, vz, temp, sm, qty, mass, rho]

    logger.info("Constructing SPH spectrum at %.2f s" %
                (time.time() - in_time))
