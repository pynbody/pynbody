...
//template<typename T>
float Wendland_kernel(SMX smx, float rs, float r2, int nSmooth)
{
	// Wendland Kernel 
...
void smDensitySym(SMX smx,int pi,int nSmooth,int *pList,float *fList, bool Wendland)
{
	...
	for (i=0;i<nSmooth;++i) {
		...
		r2 = fList[i]*ih2;
		if (Wendland) {
			rs = Wendland_kernel(smx, rs, r2, nSmooth);
		} 
		else {
			rs = cubicSpline(smx, rs, r2);
		}
			rs *= fNorm;
...
void smCurlQty(SMX smx,int pi, int nSmooth,int *pList,float *fList, bool Wendland)
{
	...
	for (j=0;j<nSmooth;++j) {
		...
		// Kernel gradient
		if (Wendland) {
			rs = Wendland_gradient(q, rs, r);
		}
		else {
			rs = cubicSpline_gradient(q, rs, ih, r, ih2);
		}
