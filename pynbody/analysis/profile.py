...
def v_circ(p, grav_sim=None):
    """Circular velocity component, i.e. rotation curve generated by particles included in the profile.

    For all particles in the same region as the profile, use v_circ_total."""
...
@Profile.profile_property
def v_circ_total(p):
    """Total circular velocity, i.e. rotation curve generated by all particles in the region

    This includes particles that are not part of the requested profile, out to 10 times the
    maximum radius of the profile. For gravity generated only by particles in the current profile, use v_circ."""
    grav_sim = p.sim.ancestor[pynbody.filt.Sphere(p['rbins'].max()*10)]
    return v_circ(p, grav_sim)
...
@Profile.profile_property
def omega(p):
    """Circular frequency Omega = v_circ_total/radius (see Binney & Tremaine Sect. 3.2)"""
    prof = p['v_circ_total'] / p['rbins']
