...
class Cosmology:
    ...
    def Exp2Hub(self,dExp):
        ...
        return (self.dHubble0*math.sqrt(self.dOmegaM*dExp
                                        ...
                                        + self.dOmegaRad
                                        + self.dQuintess*dExp*dExp*math.sqrt(dExp)
	...
	dHubble0 = self.dHubble0;
        ...
        dDeltaOld = dExpNew;	# old change in interval 
        dUpper = 1.0e38; # bounds on root 
        dLower = 0.0;
...
#* This function integrates the time dependence of the "drift"-Hamiltonian.
	...
	if (not self.bComove): return(dDelta);
	...
	if (not self.bComove): return(1.0);
	else: 
	    dExpOld = 0.0;
...
# delta[1] => deltadot
    def GrowthFacDeriv(self,dlnExp, dlnDelta, dlnDeltadot):
        ...
        dlnDeltadot[1] = (-dlnDelta[1]*dlnDelta[1]
                          - dlnDelta[1]*(1.0 + self.ExpDot2( dExp)/(dHubble*dHubble))
...
#        , 2, 
#        , dDEnd,  nSteps);
        integrator = integrator.set_initial_value(2,dlnExpStart,dDStart, dlnExp)
